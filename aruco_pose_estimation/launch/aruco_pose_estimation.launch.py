"""Modified by:
Anuj Pai Raikar - for JLG Industries
"""
"""
CHANGE LOG:

June 12, 2024
1. Added the depth profile resolution to the config during launch:  
Set to 1280x720x30 as default
"""
...
def generate_launch_description():
        ...
        parameters=[
            {
                ...
                "markers_visualization_topic": LaunchConfiguration(
    ...
    camera_feed_depth_node = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            # "depth_module.depth_profile": "848x480x30", #Added for depth stream resolution
    ...
    camera_feed_node = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            "enable_color": "true",
            # "rgb_camera.color_profile": "848x480x30", #Added for color stream resolution
        }.items(),
        condition=UnlessCondition(LaunchConfiguration("use_depth_input")),
    )

    rviz_file = PathJoinSubstitution(
        [FindPackageShare("aruco_pose_estimation"), "rviz", "cam_detect.rviz"]
    )

    rviz2_node = Node(package="rviz2", executable="rviz2", arguments=["-d", rviz_file])
    ...
    return LaunchDescription(
        [
            ...
            # Nodes
            aruco_node,
            camera_feed_depth_node,  # Comment when not using RealSense with ROS wrapper  - i.e., when camera launched as usb_cam or as ros bag
            camera_feed_node,  # Comment when not using RealSense with ROS wrapper - i.e., when camera launched as usb_cam or as ros bag
            rviz2_node,
        ]
    )
